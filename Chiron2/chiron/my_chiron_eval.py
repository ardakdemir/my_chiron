...
from chiron.chiron_input import read_data_for_eval,read_data_for_eval_pickle
...
def norm_edit_distance(seq1,seq2):
    ...
    norm  = edits[-1][-1]/(max(len(seq1),len(seq2)))
    return norm
...
def evaluation():
    ...
    with tf.train.MonitoredSession(session_creator=tf.train.ChiefSessionCreator(config=config)) as sess:
        ...
        for i in trange(0, reads_n, FLAGS.batch_size, desc="Output",position = 4):
            ...
            mapped_predict = map_predict(predict_read[0])
            unique = unique[0]
            tr = instances[i]["nucleotides"]
            edit_distance1 = edit_distance(tr,predict_read[0])
            edit_distance2 = norm_edit_distance(tr,mapped_predict)
            edit_dists.append(edit_distance2)
        ...
        aver_error = avg(edit_dists)
        mean,std  = get_stats(edit_dists)
        print("Average normalized edit distance : %0.3f\n"%mean)
        print("Standard deviation : %0.3f\n"%std)
        print("Saving information in %s"%out_file)
        #for edit in edit_dists:
        #    out.write(str(edit)+"\n")
        out.write("Average normalized edit distance : %f\n"%aver_error)
        out.write("Standard deviation : %0.3f\n"%std)
        out.close()
