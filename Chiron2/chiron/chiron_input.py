...
class DataSet(object):
    def __init__(self,
                 ...
                 event_length,
                 label,
                 label_length,nuc = None,y_vec=None,segments = None,
                 for_eval=False,
                 ):
        """Custruct a DataSet."""
        if for_eval == False:
...
def read_data_for_eval_pickle(file_path,example_num=100,
					   start_index=0,
                       step=20,
	                   seg_length=300,
                       sig_norm=True,
    ...
    if not file_path.endswith('.pk'):
        raise ValueError('A .pk file is required.')
    else:
        event = list()
        event_len = list()
        label = list()
        ...
        y_vecs = list()
        #print(file_path)
        segmented_data = pickle.load(open(file_path,"rb"))
        ...
        try:
            ...
            for i in range(100):
                print("BUramismdiasmdiosa")
        instances = list()
        for i in keys:
            #print(segmented_data[i])
            try:
                ins = segmented_data[i]
                try:
                    segment_sig = ins['x_data']
                    #print(segment_sig)
                    segment_len = len(ins['x_data'])
                    #padding(segment_sig,seg_length)
                    event.append(segment_sig)
                    event_len.append(segment_len)
                except:
                    print("Hata xde")
                segment = ins["segments"]
                nuc = ins["nucleotides"]
                y_vec = ins["y_vec"]
                nucs.append(nuc)
        ...
        evaluation = DataSet(event=event,
							 ...
							 nuc = nucs,y_vec=y_vecs,segments = segments,
							 for_eval=True)
